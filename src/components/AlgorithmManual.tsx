...
#### **2. 求解极限平衡胜率 $P^*$**
由于能量获取率 $\\frac{dE}{dN}$ 是关于面板均值胜率 $\\overline{P}$ 的折线分段函数，求解需分两种情况（针对当前抵扣系数 $k$）：

* **情况 A：假设极限胜率 $\\le$ 面板均值胜率（绝大多数情况）**
  代入左侧折线公式 $\\frac{dE}{dN} = \\frac{M_{E/P}}{\\overline{P}} P^*$，解得：
  $$P^*_A = \\frac{15}{30 + \\dfrac{M_{E/P}}{k \\cdot \\overline{P}}}$$
  *(若算出 $P^*_A \\le \\overline{P}$，则 $P^* = P^*_A$；否则执行情况 B)*

* **情况 B：假设极限胜率 $>$ 面板均值胜率（极端低胜率高牌子玩家）**
  代入右侧折线公式，令斜率 $m = \\frac{6 - M_{E/P}}{1 - \\overline{P}}$，解得：
